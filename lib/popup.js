...
elStart.addEventListener("click", async () => {
  ...
  const ensureContentReady = async () => {
    try {
      const res = await API.tabs.sendMessage(CURRENT_TAB.id, { type: "PING" });
      return !!res?.ok;
    } catch {
      return false;
    }
  ...
  let ready = await ensureContentReady();
  if (!ready) {
    try {
      await API.scripting.executeScript({
        target: { tabId: CURRENT_TAB.id, allFrames: true },
        files: ["lib/content.js"],
      });
      ready = await ensureContentReady();
    } catch (e) {
      log("İçerik script enjekte edilemedi: " + (e?.message || e));
    }
  }

  if (!ready) {
    log("İçerik script hazır değil. Sayfayı yenileyip tekrar deneyin.");
    return;
  }
