...
async function openModal(card, cfg) {
  ...
  await clickAndWait(clickTarget, {
    afterClickDelay: cfg.afterProductClickDelay ?? 250,
  });
...
async function scrapeSection(section, cfg) {
  ...
  for (let i = 0; RUNNING && i < cards.length; i++) {
    const card = cards[i];
    overlaySet(`[${categoryName}] ${i + 1}/${cards.length}`);

    if (cfg.productEnsureVisible) {
      card.scrollIntoView({ behavior: "smooth", block: "center" });
      await sleep(cfg.productAfterScrollPause ?? 120);
    }

    if (cfg.useModal) {
      const modal = await openModal(card, cfg);
      if (!modal) {
        await sleep(cfg.betweenItemsDelay ?? 120);
        continue;
      }
